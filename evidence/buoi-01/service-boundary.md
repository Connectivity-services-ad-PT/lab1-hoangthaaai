...
- Cung cấp thông tin camera, trạng thái camera và kết quả kiểm tra sức khỏe service cho các thành phần được phép sử dụng.
...
- Cung cấp cơ chế kiểm tra tình trạng hoạt động và kết nối của camera.
...
- Giới hạn quyền truy cập theo vai trò sử dụng (Admin, system).  
...
- Không điều khiển trực tiếp camera (ví dụ PTZ) nếu chưa có phạm vi điều khiển riêng được thống nhất.
...
- Frontend/Ứng dụng quản trị: lấy danh sách camera, chi tiết camera và thực hiện thao tác quản lý trong phạm vi được phép.
- Service điều phối và lưu trữ video: gọi để lấy thông tin camera và trạng thái hoạt động.
- Thiết bị camera: gửi trạng thái hoạt động hoặc thông tin đăng ký ban đầu.
...
```mermaid
flowchart LR
    User[Frontend/Quản trị] -->|Yêu cầu quản lý| CameraService[Camera Service]
